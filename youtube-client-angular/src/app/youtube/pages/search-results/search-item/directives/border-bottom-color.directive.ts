...
export class BorderBottomColorDirective implements OnInit {
  ...
  ngOnInit(): void {
    ...
    const daysAfterPublication: number = Math.ceil(
      (currentDate.getTime() - publicDate.getTime()) / MILLISECONDS_PER_DAY,
    );

    const borderColor: string =
      daysAfterPublication >= 183
        ? EBorderColor.red
        : daysAfterPublication >= 30 && daysAfterPublication < 183
        ? EBorderColor.yellow
        : daysAfterPublication >= 8 && daysAfterPublication < 30
        ? EBorderColor.green
        : EBorderColor.blue;
